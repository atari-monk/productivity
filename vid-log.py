...
class LogEntry:
    def __init__(self, video_link: str, start_time: datetime, end_time: Optional[datetime] = None, note: str = ""):
    ...
    def format_entry(self) -> str:
        ...
        return f"{date_str} {time_range} {duration_str} {self.video_link}\n{self.note}".strip()
...
class LogManager:
    ...
    def start_entry(self, video_link: str, note: str = ""):
        active_entry = self.file_handler.get_active_entry()
        if active_entry:
            print(f"There is already an active entry for video: {active_entry.video_link}")
            return
        
        new_entry = LogEntry(video_link, datetime.now(), note=note)
        self.file_handler.write_entry(new_entry)
        print(f"Started logging for video: {video_link}")
...
def main():
    log_path = Path("vid-log-2025.txt")
    ...
    if not args:
        print("Usage: vid_log <command> [args]")
        print("Commands:")
        print("  vid_link [note] - Start tracking video with optional note")
        print("  note <note> - Add note to current video entry")
        print("  end [note] - End current video entry with optional note")
        return
    ...
    if command == "note":
        ...
        video_link = args[0]
        note = ' '.join(args[1:]) if len(args) > 1 else ""
        log_manager.start_entry(video_link, note)
